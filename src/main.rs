...
async fn main() -> Result<(), Error> {
    ...
    let ultimora_cringe: HashSet<i64> = fs::read_to_string("ultimora_cringe.txt")
        ...
        .map(|x| x.trim().parse().expect("One of the ids is malformed"))
        .collect();
            ...
            Ok(update) => {
                if let UpdateKind::Message(message) = &update.kind {
                    if let Some(forward) = &message.forward {
                        let should_delete = match &forward.from {
                            ForwardFrom::Channel { channel, .. } => {
                                ultimora_cringe.contains(&channel.id.into())
                            }
                            ForwardFrom::User { user } => {
                                ultimora_cringe.contains(&user.id.into())
                            }
                            _ => false,
                        };
